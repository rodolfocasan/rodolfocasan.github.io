// src/app/apps/mobile/another-chat-room/Components/Introduction.jsx
'use client';
import React, { useState, useEffect } from 'react'
import { FaComments, FaClock, FaUsers, FaWifi, FaServer, FaBell, FaMobile, FaAndroid } from 'react-icons/fa'
import { IoIosChatbubbles } from 'react-icons/io'
import { MdTimelapse, MdAutoDelete, MdSettings } from 'react-icons/md'
...
function Introduction() {
    const favicon_url = "https://raw.githubusercontent.com/rodolfocasan/another-chat-room/v1.0.1/assets/icon.png"
    ...
    useEffect(() => {
        const fetchLogo = async () => {
            try {
                ...
                console.warn('Error al fetchear el logo de Another Chat Room:', error)
    ...
    const mainFeatures = [
        {
            ...
            title: "Salas Temporales",
            description: "Crea salas de chat que expiran automáticamente"
        },
        {
            icon: <MdTimelapse className="text-2xl" />,
            title: "Tiempo Real",
            description: "Mensajería instantánea con WebSockets"
        },
        {
            icon: <FaUsers className="text-2xl" />,
            title: "Gestión de Salas",
            description: "Administra tus salas y únete a otras existentes"
        },
        {
            icon: <FaWifi className="text-2xl" />,
            title: "Reconexión Auto",
            description: "Manejo inteligente de pérdida de conexión"
        }
    ]
    ...
    const detailedFeatures = [
        {
            category: "Gestión de Salas",
            items: [
                { icon: <IoIosChatbubbles />, text: "Crear salas temporales" },
                { icon: <FaUsers />, text: "Unirse a salas existentes" },
                { icon: <MdAutoDelete />, text: "Expiración automática" },
                { icon: <MdSettings />, text: "Información de propietario" }
            ]
        },
        {
            category: "Mensajería",
            items: [
                { icon: <MdTimelapse />, text: "Mensajes en tiempo real" },
                { icon: <FaClock />, text: "Timestamps automáticos" },
                { icon: <RiMessage3Line />, text: "Límite de 500 caracteres" },
                { icon: <FaBell />, text: "Historial de sesión" }
            ]
        },
        {
            category: "Conexión",
            items: [
                { icon: <FaWifi />, text: "Reconexión automática" },
                { icon: <FaServer />, text: "Cambio de servidor dinámico" },
                { icon: <MdTimelapse />, text: "Estados visuales" },
            ...
            <div className="relative z-10 container mx-auto px-4 py-16 lg:py-24">
                ...
                <div className={`text-center mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            ...
                            <div className="relative">
                                ...
                                <div className="relative bg-gray-900 rounded-2xl p-2">
                                    ...
                                    <img
                                        ...
                                        onError={() => {
                                            setLogoLoaded(true)
                                            console.warn('Error al cargar el logo de Another Chat Room')
                                        }}
                                    />
                                </div>
                            </div>
                    ...
                    <h2 className="text-xl md:text-2xl lg:text-3xl text-gray-300 font-light mb-6 leading-relaxed max-w-4xl mx-auto">
                        Aplicación de <span className="text-green-400 font-medium">chat en tiempo real</span> para Android con{' '}
                        <span className="text-blue-400 font-medium">salas temporales</span> y{' '}
                        <span className="text-purple-400 font-medium">expiración automática</span>
                    </h2>

                    {/* Descripción principal */}
                    <p className="text-gray-400 text-lg md:text-xl leading-relaxed max-w-3xl mx-auto">
                        Desarrollada con <span className="text-green-400 font-medium">React Native</span> y{' '}
                        <span className="text-blue-400 font-medium">Socket.IO</span>, esta aplicación permite crear y unirse a salas de chat
                        que se eliminan automáticamente, proporcionando una experiencia de mensajería{' '}
                        <span className="text-purple-400 font-medium">instantánea y temporal</span>.
                    </p>
                </div>
